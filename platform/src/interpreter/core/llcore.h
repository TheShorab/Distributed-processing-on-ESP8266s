...
public:
    ...
    FN_RETURN_TYPE(void)
    FN_NAME terminateExecution(const Platform::Base::Interpreter::Error &e)
    {
        Print(e.cause());
        Print(EXF);
        exit(0);
...
public:
    ...
    INDEPENDENT bool isRunAllowedOnNextBlock() const
    {
        if (_blockedScopes.empty())
            return true;
        auto &blockedScope = _blockedScopes.top();
        if (blockedScope.first <= scope.first + 1 /*&& blockedScope.second >= scope.second + 1*/)
            return false;
        return true;
    }
    ...
    INDEPENDENT void blockNextScope()
    {
        if (_blockedScopes.empty())
            goto end;
        if (_blockedScopes.top().first <= scope.first + 1)
            return;

    end:
        _blockedScopes.push({scope.first + 1,
                             scope.second});
    }
    ...
    INDEPENDENT void blockThisScope()
    {
        if (_blockedScopes.empty())
            goto end;
        if (_blockedScopes.top().first <= scope.first)
            return;
...
std::string LLCore::checkAccessToVariableAndScopizeName(const std::string &name)
{
    ...
    if (_refs.find(nameOnThisScope) == _refs.end())
    {
        ...
        nameWithScope = _refs[nameOnThisScope];
